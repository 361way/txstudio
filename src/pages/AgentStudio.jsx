                    ...
                    <aside className="space-y-4">
                        ...
                        <div className="rounded-2xl border border-[#ebe6da] bg-[#fbfaf7] p-4 shadow-[0_8px_24px_rgba(50,43,24,0.04)]">
                            ...
                            <div className="mt-4 grid grid-cols-2 gap-3">
                                ...
                                <label><span className="mb-1.5 block text-[11px] text-[#736c5d]">{t('清晰度')}</span><select value={resolution} disabled={isRunning} onChange={(event) => setResolution(event.target.value)} className={selectClass}>{videoCapability.resolutions.map((value) => <option key={value} value={value}>{videoCapability.resolutionLabels?.[value] || value}</option>)}</select></label>
                            ...
                            <p className="mt-2 text-center text-[10px] leading-4 text-gray-300">{maxShots === 'auto' ? t('Agent 将根据剧本节奏自主规划镜头数量，最多 8 个镜头') : t('将按人物数与镜头数调用图片和视频生成服务')}</p>
                        </div>
                    </aside>
