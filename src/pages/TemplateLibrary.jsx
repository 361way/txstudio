...
export default function TemplateLibrary({ onBack, onApply, theme, quota, onForcedLogout, embedded = false }) {
            ...
            <div className="max-w-6xl mx-auto px-6 py-8">
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-white">{t('模板库')}</h1>
                            <p className="text-xs text-zinc-500">{t('一键套用场景模板到图片 / 视频工具')}</p>
                        </div>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-24">
                        <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-white/5 border border-white/8 mb-3">
                            <LibraryBig className="w-7 h-7 text-zinc-600" />
                        </div>
                        <p className="text-zinc-500 text-sm">{t('暂无模板')}</p>
                    </div>
                ) : (
                        ...
                        {filtered.map((tpl) => (
                            <div key={tpl.id} className="glass-card p-5 flex flex-col group hover:shadow-glow transition-shadow">
                                <div className="flex items-center justify-between mb-3">
                                    <span className="text-xs px-2 py-0.5 rounded-md bg-white/8 text-zinc-300 border border-white/8">{t(tpl.category || '其他')}</span>
                                ...
                                <h3 className="font-semibold text-white mb-1.5">{t(tpl.name)}</h3>
                                {tpl.description && <p className="text-xs text-zinc-400 mb-3 line-clamp-2">{t(tpl.description)}</p>}
                                <div className="text-xs text-zinc-500 mb-4 mt-auto">
