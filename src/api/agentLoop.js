...
const AUTO_SHOT_LIMIT = 8;

function resolveShotPlan(maxShots) {
    if (String(maxShots).trim().toLowerCase() === 'auto') {
        return { mode: 'auto', limit: AUTO_SHOT_LIMIT };
    }
    return {
        mode: 'manual',
        limit: Math.max(1, Math.min(Number(maxShots) || 4, AUTO_SHOT_LIMIT)),
    };
}
...
export async function runScriptAgentLoop({
    ...
    const shotPlan = resolveShotPlan(maxShots);
    const safeMaxShots = shotPlan.limit;
    const run = {
        status: 'running',
        shotPlanningMode: shotPlan.mode,
    ...
    try {
        ...
        const storyboardText = await callTextModel({
            ...
            user: `${shotPlan.mode === 'auto'
                ? `基于剧本和角色表，自主判断叙事节奏、场景转换、关键动作与情绪转折，规划最合适的核心镜头数量（1 至 ${safeMaxShots} 个）。镜头要覆盖完整故事，但不要为凑数量拆分重复或无叙事价值的画面。`
                : `基于剧本和角色表生成最多 ${safeMaxShots} 个核心镜头。`}返回 {"shots":[{"title":"","scene":"画面内容与动作","camera":"景别、机位和运镜","dialogue":"对白或旁白，没有则空","characters":["角色名"],"visual_prompt":"静态分镜图中文提示词","video_prompt":"从分镜图生成视频的中文提示词，描述动作、运镜、节奏与环境变化"}]}。镜头须按叙事顺序，可生成且相互衔接。\n\n角色表：${JSON.stringify(characterSummary)}\n\n剧本：\n${script}`,
        });
