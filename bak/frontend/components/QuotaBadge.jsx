...
export default function QuotaBadge({ theme = 'dark', limits = null }) {
    ...
    const badge = (Icon, used, limit, low) => {
        const tone = low
            ? (theme === 'light' ? 'bg-red-50 text-red-600 border-red-200' : 'bg-red-950/50 text-red-300 border-red-800/60')
            : (theme === 'light' ? 'bg-[#f4f4f5] text-gray-600 border-[#ececef]' : 'bg-zinc-800/60 text-zinc-300 border-white/8');
        return (
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium border transition ${tone}`}>
