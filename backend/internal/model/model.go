...
func removeRetiredImageTemplateColumns(db *gorm.DB) error {
	migrator := db.Migrator()
	changed := false
	for _, column := range []string{"source_name", "source_url"} {
		if !migrator.HasColumn(&ImageTemplate{}, column) {
			continue
		}
		if err := migrator.DropColumn(&ImageTemplate{}, column); err != nil {
			return fmt.Errorf("删除 image_templates.%s 失败: %w", column, err)
		}
		changed = true
	}
	if !changed {
	...
	// SQLite 通过重建表来删除列，重新迁移以恢复模型声明的索引。
	if err := db.AutoMigrate(&ImageTemplate{}); err != nil {
		return fmt.Errorf("重建 image_templates 索引失败: %w", err)
	}
	return nil
}
...
func AutoMigrateAll(db *gorm.DB) error {
	...
	if err := removeRetiredImageTemplateColumns(db); err != nil {
		return err
	}
